...
<div id="backgroundContainer">
	...
	<skeleton:control name="div" presentationContext="${layoutpc}" 
		...
		<c:set var="ph2" value="${ph[1]}"/>
			<div >
				<jsp:scriptlet>
					if( context.getProperty("claseHd") != null && !context.getProperty("claseHd").equals("") ) {
						request.setAttribute("claseHd", context.getProperty("claseHd"));
					}else{
						request.setAttribute("claseHd", "padding-top40");
					}
				</jsp:scriptlet>
				<!-- Wrap Main -->	
				<div style="float: left;">
					<!-- Main -->	
					<div class="span-18 ${ claseHd}">
						<div id="test">
							<skeleton:child presentationContext="${ph1}"/>
						</div>
					</div>
					<!-- Sidebar -->
					<div class="span-5 last ${ claseHd}">
