$(document).ready(function() {	
	...
	$('.head-prev').click(function(){
		$('#preloader').stop();
		if(activeNav == "2"){
			changeHead("${imagenesSize}");
		} else if(activeNav == "1"){
			changeHead("${imagenesSize-1}");
		} else{
			changeHead(new Number(parseInt(activeNav)-2).toString());
		}
		return false;
	})
	...
	$('${navIds}').click(function(){
		$('#preloader').stop();
		var thisNavNum = $(this).attr('name');
		$('#navs-rot div').css('display','none');
		$('#cont-word'+thisNavNum).css('display','block');
		changeHead(thisNavNum);
		return false;
	});
	...
	$('#navs-rot a').mouseover(function(){
		$(this).removeClass('hidden-text');
	});
	$('#navs-rot a').mouseout(function(){
